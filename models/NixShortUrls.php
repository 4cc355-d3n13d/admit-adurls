...
class NixShortUrls extends \yii\db\ActiveRecord
{
  ...
  public function attributeLabels()
  {
    return [
      'id' => Yii::t('adurls', 'ID'),
      'user_id' => Yii::t('adurls', 'USER_ID'),
      'long_url' => Yii::t('adurls', 'LONG_URL'),
      'short_code' => Yii::t('adurls', 'SHORT_CODE'),
      'time_create' => Yii::t('adurls', 'CREATED_TIME'),
      'time_end' => Yii::t('adurls', 'TIME_END'),
      'counter' => Yii::t('adurls', 'COUNTER'),
    ];
  }
  ...
  public static function validateShortCode($code)
  {
    if (!preg_match('|^[0-9a-zA-Z]{6,6}$|', $code)) {
      throw new HttpException(400, Yii::t('adurls', 'ENTER_VALID_SHORT_CODE'));
    }

    $url = NixShortUrls::find()->where(['short_code' => $code])->one();

    if ($url === null) {
      throw new NotFoundHttpException(Yii::t('adurls', 'SHORT_CODE_NOT_FOUND') . $code);
    } else if (!is_null($url['time_end']) && date("Y-m-d H:i:s") > $url['time_end']) {
      throw new NotAcceptableHttpException(Yii::t('adurls', 'SHORT_CODE_END_TIME') . $url['time_end']);
    }
  ...
  public function checkUrl($url)
  {
    ...
    if ($curl->responseCode != 200)
      throw new HttpException(400, Yii::t('adurls', 'SHORT_CODE_ERROR_URL') . $curl->responseCode);
  }
